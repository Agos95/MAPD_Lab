         ...
         if reset = '1' then
            ...
            when s_getbyte =>
                ...
                cnt_o := cnt_o + 1;
				        ...
				        if bcnt = 0 then
                  s_word(N_BYTES*RXBITS-1 downto (N_BYTES-1)*RXBITS ) <= s_rxd; -- last location -> MSB
                  bcnt  := N_BYTES - 1;
                  state <= s_stop;
                  ready_s <= '1';
                else
                  bcnt  := bcnt - 1;
                  s_word((N_BYTES-bcnt+1)*RXBITS-1 downto (N_BYTES-bcnt)*RXBITS ) <= s_rxd; -- first locations -> LSB
                  state <= s_getbyte;
                  ready_s <= '0';
                end if; 
                we_out <= '0';
